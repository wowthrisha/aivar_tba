...
| D-36 | Secrets exposed seven times: `railway variables` dumps, `aws lambda get-function` metadata, and recursive greps over a directory containing `.env`. Root cause: secrets stored as plaintext environment variables and in a local `.env`, combined with the unsafe command being easier to type than the safe one in every instance. | Containment verified — git history clean, `.env` gitignored and untracked, all credentials rotated. Re-verified independently rather than trusted (this environment's interactive `grep` is aliased to a gitignore-aware `ugrep` wrapper that would silently skip `.env` during a recursive scan and produce a false-clean result; re-checked with `command grep`, bypassing the alias). | Fixed structurally: `scripts/scan-secrets.sh` (filename-only scan wrapper — first version's generic 40-hex-char pattern false-positived on this repo's own git-SHA references in governance docs, caught by actually running it and scoped to code paths only), `scripts/check-var.sh` (scoped variable-check wrapper, PRESENT/ABSENT + 6 chars max, demonstrated on all three platforms), `scripts/pre-commit` (5-pattern block, each demonstrated blocking a fake value with the value never printed — the previous version of this same hook echoed the matched line, a real gap found and fixed here), `app/logging_config.py::RedactSecretsFilter` (log redaction, tested against a real log line), `.github/workflows/secret-scan.yml` (CI diff scan, demonstrated against a pushed scratch branch), and a NEVER-DUMP section in `CLAUDE.md` making the safe form the documented path for all three historical vectors. AWS secrets migration to SSM SecureString **not completed**: `aivar-deploy` has zero `ssm:*`/`iam:*` permissions (`ssm:DescribeParameters` and every `iam:List*Policies` call AccessDenied, confirmed live, not assumed) — stopped per instruction rather than working around it; exact policy JSON and console steps reported to the user instead. Railway retains env vars — no equivalent store — protected by the tooling guardrails only, stated explicitly in the README rather than implying parity with SSM. **`RedactSecretsFilter` deployed and verified LIVE on both platforms (2026-08-25, commit `ff706ee`), behaviorally not just by SHA:** sent a fake `sk-proj-`-shaped value in `resource`, forced a genuine `APITimeoutError` (repeated attempts, `TIMEOUT_SECONDS=3.0` in `app/llm.py`), and read the actual platform logs. Railway (`railway logs`) and AWS CloudWatch (`aws logs filter-log-events` — this IAM identity CAN read CloudWatch, unlike the Lambda-management APIs D-23 blocks) both show `llm_degraded=true reason=timeout action_type=read resource=[REDACTED]` — the fake value never appears, only `[REDACTED]`. | Fixed (guardrails, live-verified behaviorally on both platforms); SSM migration blocked on IAM, reported not worked around |
| D-37 | A dependency incompatibility between `mcp` and `fastapi==0.115.0` was reported (`app/mcp_server.py`'s own docstring, `governance/evidence/intelligence-v6-report.md`) as "confirmed by direct reproduction, twice," and a whole `.venv-mcp`/`scripts/mcp/`/`tests_mcp/` split was built on that premise — which excluded Feature A's entire test suite from CI (`testpaths=["tests"]` never saw `tests_mcp/`; no workflow installed `mcp` or ran that directory). Found by re-verifying the prior claim during a read-only merge assessment, not by a failing test. | Both "reproductions" ran `pip install "mcp[cli]"` ALONE first, with no `fastapi` constraint present in that same resolve — pip's resolver, unconstrained, picked the newest starlette satisfying only `mcp`'s own floor (`starlette>=0.27`), landing on `1.6.0`, which genuinely is incompatible with `fastapi==0.115.0`'s ceiling (`starlette<0.39.0`). The incompatibility was real for THAT installation order; it was never inherent to the two packages together. A resolver-ordering mistake was reported as a hard package conflict. | Re-verified fresh from a clean venv: `pip install -r requirements.txt "mcp[cli]"` (one resolve, both constraints given together) settles on `starlette==0.38.6`; `from app.main import app` and `from mcp.server import MCPServer` both import in the same process; full suite green. `mcp[cli]==2.1.1` added to `requirements.txt` (a normal pinned dependency now, with the resolver-ordering explanation recorded in its own comment there and in `app/mcp_server.py`'s docstring). `tests_mcp/test_mcp_server.py` moved to `tests/test_mcp_server.py` (now covered by `testpaths=["tests"]` and CI); `scripts/mcp/_test_server.py` moved to `tests/_mcp_http_test_server.py` (leading underscore — not picked up by pytest's default collection, kept as a genuinely useful real-HTTP-server test fixture on its own merits, not as a dependency workaround); `.venv-mcp/`, `scripts/mcp/`, `tests_mcp/` deleted. 241 passed, 9 skipped (up from 236/9 — the 5 newly-covered MCP tests), 0 failed. | Fixed |
...
- **L-K** Feature E's (`GET /v1/actions/{id}/explain`) real grounding —
  whether an LLM, given the permitted structured record, actually
  refrains from inventing facts absent from it — is untested.
  `tests/test_api.py::test_explain_receives_only_the_structured_record`
  (intelligence-v6; renamed from a name claiming "grounding" — see that
  commit) proves something narrower and genuinely real: the endpoint
  withholds `action_type`/`resource`/`params` from the provider before
  any LLM call happens. It uses a fake, non-LLM provider that returns a
  fixed string regardless of input, so it has no capacity to invent
  anything, and no test in this project inspects the actual prompt text
  `OpenAIExplanationProvider._call()` sends to OpenAI either. This is a
  structural gap, not an oversight specific to Feature E: this project
  makes no live LLM calls anywhere in its test suite (`tests/test_llm.py`
  mocks the client the same way), so nothing here CAN verify a real
  model's behavior without breaking that convention. Production
  approach: a recorded-response fixture (a real, one-time-captured
  OpenAI response, replayed deterministically) built from a deliberately
