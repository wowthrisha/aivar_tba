...
| ID | Task | Definition of Done (evidence that closes it) | Box | Status |
|---|---|---|---|---|
| T-06 | scorer.py, four dimensions | pytest tests/test_scoring.py green | 2h | Not started |
...
| T-09 | llm.py adapter | test with mocked refusal proving terminal, not retried | 45m | Not started |
...
| ID | Task | Definition of Done (evidence that closes it) | Box | Status |
|---|---|---|---|---|
| T-10 | API + state machine | curl each endpoint, responses pasted | 1h | Not started |
| T-11 | Persistence | alembic current on DIRECT; app on POOLED; \dt shows 4 tables | 1h | Not started |
