...
async def compute_calibration_by_action_type(store, audit) -> dict[str, CalibrationStats]:
    ...
    D-32: previously iterated every historical confirmed/decision audit
    record and did one store.get_action() round trip per record (N+1,
    122s at 17 records against a 600+-row audit_records table). Now a
    single call to audit.calibration_outcomes(store) - a single aggregate
    SQL query on the DB-backed path (app/db_store.py), a plain in-memory
    iteration on the test-double path (app/audit.py)."""
    counts = await audit.calibration_outcomes(store)
