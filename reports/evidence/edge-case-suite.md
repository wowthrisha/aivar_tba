...
**Root cause identified and fixed** (see `progress-log/03-errors-and-fixes.md`, D-14): the `low_confidence` floor fired on ALL actions regardless of reversibility, including fully-reversible reads. Fixed by gating it on `is_mutation` (`app/risk/floors.py`) - see the code change and STEP 3 re-verification (`reports/evidence/final-demo-capture.json`) in this same commit range for the deterministic re-verification across the full confidence range.
